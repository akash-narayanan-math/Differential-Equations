...
import numpy as np
import pandas as pd
from improved_euler_method import improved_euler
import pdb
...
def runge_kutta(x0, y0, x, h):
    ...
    for i in np.arange(x0, x, h):
        ...
        k_2 = derivative(x_old + h / 2, y_old + (h / 2) * k_1)
        k_3 = derivative(x_old + h / 2, y_old + (h / 2) * k_2)
        k_4 = derivative(x_old + h, y_old + h * k_3)
        y_n = y_old + (h / 6) * (k_1 + 2 * k_2 + 2 * k_3 + k_4)
...
def main():
    ...
    x_n = 2.0
    step_size = 0.1
    list0 = improved_euler(x_0, y_0, x_n, 0.1)
    list1 = improved_euler(x_0, y_0, x_n, 0.05)
    list2 = improved_euler(x_0, y_0, x_n, 0.025)
    list3 = runge_kutta(x_0, y_0, x_n, 0.1)
    list4 = runge_kutta(x_0, y_0, x_n, 0.05)
    list5 = runge_kutta(x_0, y_0, x_n, 0.025)
